...
namespace UnityEngineAnalyzer.Language
{
    ...
    public class StructOverrideGetHashCodeAnalyzer : DiagnosticAnalyzer
    {
        ...
        private void CheckStructShouldOverride(SyntaxNodeAnalysisContext context)
        {
            ...
            bool foundOverridGetHashCode = false;
            ...
            foreach (var oneChild in declSyntax.ChildNodes())
            {
                if(oneChild is MethodDeclarationSyntax)
                {
                    ...
                    {
                        if ((oneMethodSymbol.Parameters == null || oneMethodSymbol.Parameters.Length == 0) &&
                            oneMethodSymbol.Name == "GetHashCode")
                        {
                            foundOverridGetHashCode = true;
                            break;
                        }
                    }
                }
        ...
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics => 
            ImmutableArray.Create(
                DiagnosticDescriptors.StructShouldOverrideGetHashCode
                );
    }
}
